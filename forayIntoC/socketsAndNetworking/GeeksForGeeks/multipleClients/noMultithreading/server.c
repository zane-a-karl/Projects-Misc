...
int main (int argc, char* argv[]) {
  ...
  // initialize client_socket[] to 0 so not checked
  ...
  /* Set this master socket to allow multiple connections
  Note: this is just a good habit, code will work without
   */
  ...
  // type of sockey created
  address.sin_family = AF_INET;
  address.sin_addr.s_addr = INADDR_ANY;
  address.sin_port = htons(PORT);

  // bind the socket to the localhost port 8888
  ...
  while(TRUE) {
	...
	//Echo back the message that came in
	  ...
	  // set the strict terminating NULL byte on the end
	  // of the data read
	  buffer[valread] = '\0';
